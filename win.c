...
void SetRcFromPilotRc(RECT *prc, RCRECT *prcPilot)
	{
...
void PilotText(HDC hdc, char *sz, RCRECT *prc, int fontId, int ptx)
	{
	RECT rcClip;
	int x;
	int y;
...
void DrawForm(HDC hdc, RCPFILE *prcpfile, int ifrm)
	{
	HBRUSH hbr;
	HBRUSH hbrNull;
	HPEN hpenSav;
	...
	rghfont[0] = CreateFont(-8*GetDeviceCaps(hdc, LOGPIXELSY)/72, 0, 0, 0, 400, 
		0, 0, 0, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
		DEFAULT_QUALITY, VARIABLE_PITCH|FF_DONTCARE, "Arial Narrow");
	rghfont[1] = CreateFont(-8*GetDeviceCaps(hdc, LOGPIXELSY)/72, 0, 0, 0, 700, 
		0, 0, 0, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
		DEFAULT_QUALITY, VARIABLE_PITCH|FF_DONTCARE, "Arial Narrow");
	rghfont[2] = CreateFont(-10*GetDeviceCaps(hdc, LOGPIXELSY)/72, 0, 0, 0, 700, 
		0, 0, 0, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
		DEFAULT_QUALITY, VARIABLE_PITCH|FF_DONTCARE, "Arial Narrow");
	...
	if (pform->window.frameType.cornerDiam)
		...
		RoundRect(hdc, 
			...
			pform->window.windowBounds.topLeft.x+pform->window.windowBounds.extent.x+1,
			pform->window.windowBounds.topLeft.y+pform->window.windowBounds.extent.y+1,
	...
	SetWindowOrgEx(hdc, -(pform->window.windowBounds.topLeft.x-pform->window.frameType.width), -(pform->window.windowBounds.topLeft.y-pform->window.frameType.width), &ptSav);
			...
			if (field.attr.underlined)
				...
				if (field.attr.singleLine)
					...
					if (dyStep < field.rect.extent.y)
						{
						MoveToEx(hdc, field.rect.topLeft.x, field.rect.topLeft.y+dyStep, NULL);
  						LineTo(hdc, field.rect.topLeft.x+field.rect.extent.x, field.rect.topLeft.y+dyStep);
						}
					}
				else
					{
					...
					for (dy = dyStep; dy < field.rect.extent.y; dy+=dyStep)
						{
						MoveToEx(hdc, field.rect.topLeft.x, field.rect.topLeft.y+dy, NULL);
  						LineTo(hdc, field.rect.topLeft.x+field.rect.extent.x, field.rect.topLeft.y+dy);
						}
					}
...
//			ctl.text = (CharPtr) IbOut() + cbLt;
			...
			switch (ctl.style)
				{
			case buttonCtl:
				RoundRect(hdc, ctl.bounds.topLeft.x-1, ctl.bounds.topLeft.y-1,
			...
			case popupTriggerCtl:
				...
				y = ctl.bounds.topLeft.y+(ctl.bounds.extent.y-4)/2+4-1;
				for (dy = 0; dy < 4; dy++)
					{
		...
		case frmListObj:
			...
			if (!list.attr.usable)
				break;
			Rectangle(hdc, list.bounds.topLeft.x-1, list.bounds.topLeft.y-1,
...
//			break;
		...
		case frmTitleObj:
			...
			if (pform->window.frameType.cornerDiam == 3 && pform->window.frameType.width == 2)	// dialogFrame
				...
				rcT = rc;
				PilotText(hdc, pchText, &rcT, 1, ptxInvert|ptxLeft|ptxCalcWidth);
				PatBlt(hdc, rc.topLeft.x, rc.extent.y,
					rc.extent.x, 2, BLACKNESS);
				}		
			break;
		case frmPopupObj:
			popup = *pobj->popup;
...
void PilrcPaint(HWND hwnd)
	...
	if (wZoom != 1)
		{
		StretchBlt(ps.hdc, 0, 0, rc.right, rc.bottom, hdc, 0, 0, dxScreen, dyScreen, SRCCOPY);
...
BOOL FPromptFileName(OPENFILENAME *pofn, char *szFileName, char *szTitle, char *szFilter, char *szDefExt, int ofn, BOOL fOpen)
	{
	int w;
...
VOID PromptOpenFile()
	...
	if (FPromptFileName(&ofn, szFileCur, "Open Pilot Resource Script...", szFilterSpecOpen, "RCP", OFN_FILEMUSTEXIST, fTrue))
		{
		Reload();
		}
	}
...
BOOL FAR PASCAL About(HWND hdlg, UINT wm, WPARAM wParam, LPARAM lParam)
	{
	switch (wm)
		{
	case WM_INITDIALOG:
...
VOID DoAbout(VOID)
	...
	DialogBox((HANDLE) hinstApp, MAKEINTRESOURCE(IDD_ABOUT), hwndApp, (DLGPROC) lpprocAbout);
	FreeProcInstance(lpprocAbout);
	}


void PilrcCmd(HWND hwnd, int wParam)
	{
	switch (wParam)
		{
	case ID_FILE_EXIT:
...
void PilrcInitMenuPopup(HWND hwnd, HMENU hmenu, int imenu)
	{
	switch (imenu)
		{
	case 1:	// Options
		CheckMenuItem(hmenu, ID_OPTIONS_ZOOM, (wZoom == 1 ? MF_UNCHECKED : MF_CHECKED) | MF_BYCOMMAND);
		break;
	case 2:	// Form
		{
		int ifrm;
...
int PASCAL WinMain(HINSTANCE hinst, HINSTANCE hinstPrev, LPSTR lpszCmdLine, int nCmdShow)
	...
	RECT rc;
	WNDCLASS  wndclass;
	long FAR PASCAL PilrcWndProc (HWND hwnd, unsigned wm, WPARAM wParam, LPARAM lParam);
	extern int __argc;
	extern char **__argv;
	extern int main(int iargMac, char **rgszArg);
